use crate::gfx::{vertex_layout::{VertexInput, VertexLayout}, vertex_list::{VertexList, VertexListInput}};
...
pub fn graphics_init(_app_data: AppData<Data>, graphics_cache: &mut GfxCache) -> AppEventResult<()> {
    ...
    // Print layout.component_stride()
    println!("{}", layout.component_stride());

    // Create vertex list
    let vertex_list = VertexList::new(layout.clone(), &[
    ...
    graphics_cache.create_vertex_array_from_vertex_layout("vertex array", layout);
...
pub fn render(_app_data: AppData<Data>, graphics_cache: &mut GfxCache, framebuffer: TargetBuffer) -> AppEventResult<()> {
    ...
    let vertex_array = graphics_cache.get("vertex array").unwrap();
    framebuffer.render_triangles(program, vertex_buffer, index_buffer, vertex_array, 3)?;
