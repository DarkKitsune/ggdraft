...
use super::{buffer::VertexBuffer, shader_gen::{shader_inputs::{ShaderInput, ShaderInputs, SHADER_INPUT_PREFIX}, shader_outputs::{ShaderOutputs, SHADER_OUTPUT_PREFIX}}, vertex_layout::VertexLayout};
...
impl InputLayout {
    ...
    pub(crate) fn generate_vertex_fragment_shaders(
        ...
        let (vertex_shader, fragment_inputs) = self.__generate_vertex_shader(vertex)
            .map_err(|e| anyhow::anyhow!("Failed to generate vertex shader: {}", e))?;
        let fragment_shader = self.__generate_fragment_shader(fragment_inputs, fragment)
    ...
    pub(crate) fn __generate_vertex_shader(&self, f: impl FnOnce(&ShaderInputs, &mut ShaderOutputs) -> Result<()>) -> Result<(String, ShaderInputs)> {
        ...
        let mut outputs = ShaderOutputs::new(true);
        ...
        for input in inputs.iter() {
            code += &format!("layout(location = {}) in {} {}{};\n", input.location(), input.value_type().glsl_name(), SHADER_INPUT_PREFIX, input.name());
        }

        // Add the outputs.
        for output in outputs.iter() {
            code += &format!("layout(location = {}) out {} {}{};\n", output.location(), output.value_type().glsl_name(), SHADER_OUTPUT_PREFIX, output.name());
        }
        ...
        if let Some(vertex_position) = outputs.vertex_position() {
        ...
        for output in outputs.iter() {
            if let Some(expression) = output.expression() {
                code += &format!("{}{} = {};\n", SHADER_OUTPUT_PREFIX, output.name(), expression);
            }
        }
        ...
        let fragment_inputs = ShaderInputs::with_inputs(
            outputs
                .iter()
                .map(|output| ShaderInput::new(output.name(), output.value_type().clone(), output.location()))
                .collect(),
        );
    ...
    pub(crate) fn __generate_fragment_shader(&self, inputs: ShaderInputs, f: impl FnOnce(&ShaderInputs, &mut ShaderOutputs) -> Result<()>) -> Result<String> {
        // Create the shader outputs.
        let mut outputs = ShaderOutputs::new(false);
